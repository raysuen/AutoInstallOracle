...
#   	please unzip database to the base dirctionary first.
#	 	put the script in base directory.
#		put the blinsl in the base dirctory. 
#################################################################################
#
#################################################################################
	...
	if [ "${Inchar:-None}" == "None" ];then
		...
		do
			...
			if [ ! ${Inchar} ];then
				...
				continue
			elif [ ${Inchar} -ne 1 -a ${Inchar} -ne 2 ];then
				echo "You must enter 1 or 2 to choose the character."
				continue
...
InstallRPM(){
	...
	dnf -y install unzip net-tools bc binutils compat-openssl10 elfutils-libelf glibc glibc-devel ksh libaio libXrender libX11 libXau libXi libXtst libgcc libnsl libstdc++ libxcb libibverbs make smartmontools sysstat ipmiutil libnsl2 libnsl2-devel net-tools unixODBC libstdc++ libstdc++-devel libaio-devel libnsl2-devel
	...
	[ `ls ${basedir}/libnsl* 2>/dev/null | wc -l` -eq 1 ] && ls ${basedir}/libnsl* | awk -v rpmpackage="" '{rpmpackage=$NF" "rpmpackage}END{print "dnf -y localinstall "rpmpackage" --nogpgcheck"}' | bash 
	[ -f ${basedir}/libpthread_nonshared.a ]&& cp ${basedir}/libpthread_nonshared.a /usr/lib64/
	while true
	do
		if [ `rpm -q unzip net-tools bc binutils compat-openssl10 elfutils-libelf glibc glibc-devel ksh libaio libXrender libX11 libXau libXi libXtst libgcc libnsl libstdc++ libxcb libibverbs make smartmontools sysstat ipmiutil libnsl2 libnsl2-devel net-tools unixODBC libstdc++ libstdc++-devel libaio-devel libnsl2-devel --qf '%{name}.%{arch}\n' | grep "not installed" | wc -l` -gt 0 ];then
			echo -e "${c_yellow}RPM not intalled list${c_end}:"
			rpm -q unzip net-tools bc binutils compat-openssl10 elfutils-libelf glibc glibc-devel ksh libaio libXrender libX11 libXau libXi libXtst libgcc libnsl libstdc++ libxcb libibverbs make smartmontools sysstat ipmiutil libnsl2 libnsl2-devel net-tools unixODBC libstdc++ libstdc++-devel libaio-devel libnsl2-devel --qf '%{name}.%{arch}\n' | grep "not installed"
...
CreateGUAndEditprofile(){
	...
	if [ `egrep "oracle" /etc/passwd | wc -l` -eq 0 ];then
		/usr/sbin/useradd -u 54321 -g oinstall -G dba,backupdba,dgdba,kmdba,oper oracle
		if [ $? -ne 0 ];then
	...
	echo "oracle:Test#8378" | chpasswd
	if [ $? -ne 0 ];then
		echo "Oracle is not existing."
		exit  92
	fi
...
InstallRdbms(){
	if [ -f ${basedir}/database/install/unzip ];then
		if [ ! -L /u01/database/install/unzip ];then
			mv ${basedir}/database/install/unzip ${basedir}/database/install/unzip.`data +"%Y%m%d"`
			ln -s /usr/bin/unzip  /u01/database/install/unzip
		fi
	fi
	#[ -f ${basedir}/database/install/unzip ]&& mv ${basedir}/database/install/unzip ${basedir}/database/install/unzip.`data +"%Y%m%d"`; ln -s /usr/bin/unzip  /u01/database/install/unzip
	...
	echo ' '
	echo ' '
	echo -e "you use the command to get information about installation:${c_yellow} tail -f ${basedir}/install.log${c_end}"
	sleep 1m
	echo ' '
	while true
	do
