...
EditDbca21CspFiles(){
    ...
    echo 'initParams=undo_tablespace=UNDOTBS1,enable_pluggable_database=true,sga_target='${sga}'MB,db_block_size=8192BYTES,nls_language=AMERICAN,dispatchers=(PROTOCOL=TCP) (SERVICE=orclXDB),diagnostic_dest={ORACLE_BASE},control_files=("{ORACLE_BASE}/oradata/{DB_UNIQUE_NAME}/control01.ctl", "{ORACLE_BASE}/oradata/{DB_UNIQUE_NAME}/control02.ctl"),remote_login_passwordfile=EXCLUSIVE,audit_file_dest={ORACLE_BASE}/admin/{DB_UNIQUE_NAME}/adump,processes=1500,pga_aggregate_target='${pga}'MB,nls_territory=AMERICA,local_listener=LISTENER_ORCL,open_cursors=300,compatible=21.0.0,db_name='${orasid}',audit_trail=db ' >> ${basedir}/dbca.rsp
